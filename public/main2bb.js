...
function emomShowFinishModal(){
  ...
  var b1=document.createElement('button');b1.style.cssText='width:100%;background:var(--accent);color:#fff;border:none;border-radius:10px;font-family:inherit;font-size:15px;padding:14px;cursor:pointer;margin-bottom:10px;';b1.textContent='NEUES EMOM STARTEN';b1.onclick=function(){modal.remove();emomSaveAndReset();showEmomSetup(true);};
  var b2=document.createElement('button');b2.style.cssText='width:100%;background:none;border:1px solid var(--border);color:var(--text);border-radius:10px;font-family:inherit;font-size:15px;padding:14px;cursor:pointer;margin-bottom:10px;';b2.textContent='WORKOUT BEENDEN';b2.onclick=function(){modal.remove();emomSaveAndReset();var ss=document.getElementById('start-screen');if(ss)ss.style.display='flex';};
