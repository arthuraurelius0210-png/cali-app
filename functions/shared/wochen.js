...
// Montag 00:00 der Woche von d — im Browser aus weekly.js, auf dem Server (require) eigene Kopie
function wochenMonday(d){
  if(typeof weeklyMonday === 'function') return weeklyMonday(d);
...
function wochenIndex(d){
  var n = Math.round((wochenMonday(d || new Date()) - WOCHEN_EPOCH) / (7 * 86400000));
...
// Belohnung abholen: der Server rechnet die Wochen-Challenge mit denselben Daten nach
// (functions/lib/rewards.js, Typ weekly) und schreibt XP und Diamanten gut. wochenSave()
// schickt vorher den Stand (Check-ins, getauscht) ins users-Dokument.
var _wochenClaiming = false;
function wochenClaim(){
  var ch = wochenChallenge(), st = wochenCurrent();
  if(st.claimed || _wochenClaiming || wochenProgress() < (ch.target || 1)) return;
  _wochenClaiming = true;
  wochenSave();
  earnReward('weekly', st.week, {label:'Wochen-Challenge: ' + ch.title}, function(res){
    _wochenClaiming = false;
    if(!res || (res.ok === false && !res.already)) return;
    st.claimed = true;
    wochenSave();
    wochenCloseSheet();
    ...
    showCelebrationOverlay({
      ...
      note: res.ok ? '+' + (res.xp || 0) + ' XP · +' + (res.diamonds || 0) + ' Diamanten' : ''
    });
  });
}

function wochenSwapApply(){
...
function wochenSwap(payWith){
  ...
  if(payWith === 'diamonds'){
    // Diamanten bucht der Server ab (wallet.js), Flammen bleiben auf dem Gerät
    spendDiamonds('swap', function(ok){ if(ok) wochenSwapApply(); });
    return;
  }
...
// Für die Cloud Functions (functions/lib/rewards.js): Pool und Rotation, sonst nichts.
if(typeof module !== 'undefined' && module.exports){
  module.exports = {WOCHEN_CHALLENGES: WOCHEN_CHALLENGES, WOCHEN_EPOCH: WOCHEN_EPOCH, wochenIndex: wochenIndex, wochenAltIndex: wochenAltIndex, wochenMonday: wochenMonday};
}
